...
func GenerateData(data *DataDictionaryEntry, sourceId int, catchAllCohortId int, wg *sync.WaitGroup, ch chan *DataDictionaryResult) {
	...
	if data.ValueStoredAs == "Number" {
		...
		log.Print("Generate histogram for Concept id.", data.ConceptClassId)
		...
		log.Print("Generate bar graph for Concept id.", data.ConceptClassId)
		...
		log.Print("Concept id", data.ConceptClassId, "not part of histogram or bar graph classes. Set Default Empty Object")
